...
completed.addEventListener('click', async () => console.log(await getList('finished'), completed));
empty(parent)
const item = el('div');
...
for (let i = 0; i < item.length; i += 1) {
  const head = el('h2')
  haus.classList.add('catItem__head');
  item.appendChild(head);
  const section = el('section');
  sec.classList.add('catItem__section');
  item.appendChild(section);
  const hhh = el('h3');
  vef.classList.add('catItem__h3');
  sec.appendChild(hhh);
  const par = el('p');
  fj.classList.add('catItem__par');
  sec.appendChild(par);
}
console.log('catItem');
}
